...
const CartPage = () => {
    const { cartDetails, cartCount, incrementItem, decrementItem } = useShoppingCart()

    if (!cartDetails || !cartCount) notFound()

    const cartProducts = Object.keys(cartDetails).map(key => products.find(product => product.sku === cartDetails[key].id))
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 w-full">
                                            <div className="md:col-span-2 md:py-4">
                                                <div className="flex flex-col max-[500px]:items-center gap-3">
                                                    ...
                                                    <h6 className="font-medium text-base leading-7 text-gray-600 transition-all duration-300 group-hover:text-df-yellow">{formatCurrency(cartDetails[product.sku].price)}</h6>
                                            ...
                                            <div className="flex items-center py-4 max-[500px]:justify-center h-full max-md:mt-3">
                                                <div className="flex items-start h-full">
                                                    ...
                                                    <input type="text"
                                                        ...
                                                        value={cartDetails[product.sku].quantity} disabled />
                                                    <button
                                            ...
                                            <div className="flex items-start md:py-8 max-[500px]:justify-center md:justify-end max-md:mt-3 h-full">
                                                <p className="font-bold text-lg leading-8 text-gray-600 text-center transition-all duration-300 group-hover:text-df-yellow">{formatCurrency(cartDetails[product.sku].value)}</p>
                                            </div>
                                        </div>
                                    </div>
                                )
